# tests\downloader\test_downloader_utils.py

from lowmo.core.downloader.downloader_utils import detect_source, reconstruct_hf_url

CIVITAI_DOWNLOAD_URL = "https://civitai.com/api/download/models/2499141"


def test_detect_civitai_source():
    assert detect_source(CIVITAI_DOWNLOAD_URL) == ("civitai",str)


def test_detect_huggingface_source():
    url = "https://huggingface.co/user/repo/resolve/main/model.safetensors"
    assert detect_source(url) == ("huggingface",str)


def test_detect_unknown_source():
    assert detect_source("https://example.com/model.bin") == ("unknown","unknown")
